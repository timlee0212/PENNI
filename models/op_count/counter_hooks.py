###################################################################
#   Provide profiling tools for counting FLOPS and parameters
#   Code adapted from XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
#   Modification: Add support to decompose conv
#
#   UNFINISHED RESEARCH CODE
#   DO NOT DISTRIBUTE
#
#   For ICML 2020 Code Submission
...
def count_convNd(m, x, y):
    ...
    if kernel_ops == 1:
        total_ops = x.shape[1] * x.shape[2] * torch.sum(m.weight!=0)
    else:
...
### ================== Modification Start =================
# Decomposed Convolution Computation
def count_decomp_convNd(m, x, y):
    x = x[0]

    base_size = m.basis.shape[1]
    num_basis = m.basis.shape[0]

    #B x Cin x W x H
    #Intermedia FeatureMap Size
    mid_size = y.shape[2] * y.shape[3]

    #bias_ops = 1 if m.bias is not None else 0

    # # N x Cout x H x W x  (Cin x Kw x Kh + bias)
    conv_ops = mid_size * (m.in_channels  * num_basis )  * base_size
    combing_ops = torch.sum(m.coefs!=0) * mid_size      #Assuming The sparsity can be utilized.

    total_ops = conv_ops + combing_ops
